# The Reading Room — Frontend High-Level Design
...
**The Reading Room**, a single-user personal book tracker that lets a reader track books across three reading states, organize them with tags, and view an at-a-glance dashboard of reading activity. This document covers the **frontend architecture only**; the backend is a pre-built Express REST API with JWT authentication.
...
**Protected Area** — Dashboard, Books list, Add/Edit Book. Wrapped in a shared app shell (left Sidebar for navigation at desktop widths, a compact top Navbar with the same links on mobile) and guarded by route protection (Section 10).

**Shared Layout** — Root layout (fonts, global styles, providers), Auth layout (public), Dashboard layout (protected shell: Sidebar/Navbar, content container).
...
- **Design tone:** Warm editorial / library — parchment background, deep ink text, serif display type (Lora) for headings and titles against a neutral sans body. No gradients anywhere; the three reading statuses (want-to-read/reading/completed) map to solid "book cover" tones (gold, oxblood, forest) that double as the app's only accent colors, echoed everywhere as thin ribbon/underline accents rather than filled color blocks.
- **Navigation:** A persistent left sidebar (icon + label, active state marked by a left accent bar) carries primary navigation at desktop widths, replacing the earlier top navbar; a compact top bar with the same links takes over on mobile.
- **Design tokens:** Spacing scale, the ink/parchment neutrals, and the three status colors are defined once as CSS custom properties (`app/globals.css`) so every component (badges, buttons, cards) draws from the same tokens rather than ad hoc values.
- **Card language:** List/card surfaces (`shelf-card`) use square-ish corners, a hairline ink border, and a bookmark-style accent ribbon that grows in along the left edge on hover — deliberately restrained (a few pixels of lift, no glow/scale) rather than a bouncy SaaS hover treatment.
