...
export const booksApi = {
  list: async (filters?: BookFilters) => {
    const raw = await apiClient.get<RawBook[]>(
      `/books${toQueryString(filters)}`,
    );
    return raw.map(toBook);
  },
  create: async (input: BookInput) =>
    toBook(await apiClient.post<RawBook>("/books", input)),
  ...
  dashboardStats: async () => {
    ...
    const stats: DashboardStats = {
      ...raw,
      recentBooks: raw.recentBooks.map(toBook),
    };
    return stats;
  },
};
